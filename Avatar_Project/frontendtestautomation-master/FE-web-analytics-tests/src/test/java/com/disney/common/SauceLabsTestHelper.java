...
public class SauceLabsTestHelper implements SauceOnDemandSessionIdProvider {
	 ...
	 *
	 * @param testObjectDriver instantiated TestObject driver
	 */
	...
	/**
	 * Constructor for a SauceLabs helper. A username and access key is required in
	 * order to set the job status on the dashboard.
	 *
	 * @param sauceLabsDriver instantiated SauceLabs driver
	 * @param sauceUsername   SauceLabs username
	 * @param sauceKey        SauceLabs access key
	 */
	...
	/**
	 * Checks the json from the SauceREST to see if the job has already been marked
	 * as a failure.
	 *
	...
	/**
	 * Adds a link to the video recorded on the SauceLabs. A job token is created in
	 * order to preserve more sensitive information from being displayed in the URL.
	 *
	 * @param scenario the current scenario
	 */
	...
	/**
	 * Creates a job token that is used to embed the video captured from SauceLabs
	 */
